...
# Instead of using this function, try using get_profile function in the BlueSky API
    def followers_and_following(self):
        ...
        if self.extract_attributes() is not None:
...
class Build():
    ...
    def build_network_feed(self,G):
        ...
        if len(self.current_tags) >0 and len(self.tags)>0:
           for ct,ts in zip(self.current_tags,self.tags):
               G.add_node(ct)
               for t in ts:
                   if t is not None:
                       G.add_nodes_from(t)
                       G.add_edges_from([ct,tag] for tag in t)
        #plt.show()
...
m = Mapping(jwt,init_feed, client,g1,g2)
m.rec_function()
#%% test statements
